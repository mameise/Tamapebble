...
#include "hw.h"
#include "hal.h"
#include <string.h>  /* for memcpy in cpu_get_flat_state/cpu_set_state */
...
/* Pebble fork additions ---------------------------------------------------
 * cpu_get_flat_state / cpu_set_state / cpu_init_from_state allow the host
 * application to serialize and restore the full CPU state (registers,
 * timer state, RAM, interrupts) to/from persistent storage. */
flat_state_t cpu_get_flat_state(void)
{
	...
	saveState.np = np;
	saveState.sp = sp;
	saveState.flags = flags;

	saveState.tick_counter = tick_counter;
	saveState.clk_timer_2hz_timestamp = clk_timer_2hz_timestamp;
	saveState.clk_timer_4hz_timestamp = clk_timer_4hz_timestamp;
	saveState.clk_timer_8hz_timestamp = clk_timer_8hz_timestamp;
	saveState.clk_timer_16hz_timestamp = clk_timer_16hz_timestamp;
	saveState.clk_timer_32hz_timestamp = clk_timer_32hz_timestamp;
	saveState.clk_timer_64hz_timestamp = clk_timer_64hz_timestamp;
	saveState.clk_timer_128hz_timestamp = clk_timer_128hz_timestamp;
	saveState.clk_timer_256hz_timestamp = clk_timer_256hz_timestamp;
	saveState.prog_timer_timestamp = prog_timer_timestamp;
	saveState.prog_timer_enabled = prog_timer_enabled;
	saveState.prog_timer_data = prog_timer_data;
	saveState.prog_timer_rld = prog_timer_rld;

	saveState.call_depth = call_depth;
	saveState.cpu_halted = cpu_halted;
...
void cpu_set_state(const flat_state_t *state)
{
	...
	/* Stefan's fix: make sure buttons are released after loading state.
	 * Otherwise a pressed-button state from the previous session could
	 * stick around and look like a constantly-held button. */
	hw_set_button(BTN_LEFT, BTN_STATE_RELEASED);
	hw_set_button(BTN_MIDDLE, BTN_STATE_RELEASED);
	hw_set_button(BTN_RIGHT, BTN_STATE_RELEASED);
}
/* --- end Pebble fork additions ----------------------------------------- */
...
/* Pebble fork addition: initialize the CPU from a previously-saved flat
 * state instead of doing a fresh reset. Used to resume a Tamagotchi after
 * the watch has been restarted. */
